...
int main(){
    ...
    BmodeClass *currentScanline = scanlineHead;//used to keep track of which object is the current scanline
    
    //Iteratively build up a BmodeClass linked list
    dataBuffer *current = dataHead;
    for (int y = 1; y < parameters->getNumScanline(); y++){
        current = current->next;//Assign the newly created object to the pointer next of currentScanline
        //Advance the pointer currentScanline to the next entry
        currentScanline->next = new BmodeClass(parameters, current,y);
